...
def read_highres(opt):
    # x = img.imread('%s' % (opt.input_name))
    # print(x.shape)
    # x = np2torch(x,opt)
    # x = x[:,0:3,:,:]
    x = tifffile.imread('%s' %(opt.highres))  # (张数，高，宽)
    # print(x.shape)
    x = np2torch(x, opt)
    x = x[:, 0:3, :, :, :]
    return x
...
#add
def adjust_scales2image_high(high_, opt):
    #opt.scale1 = min(opt.max_size / max([real_.shape[2], real_.shape[3], real_.shape[4]]),1)
    opt.scale_high = opt.max_size / max([high_.shape[2], high_.shape[3], high_.shape[4]])
    # print(opt.scale1)
    high = imresize(high_, opt.scale_high, opt)
    opt.stop_scale = opt.train_stages - 1
    # print(opt.stop_scale)
    opt.scale_factor = math.pow(opt.min_size / (min(high.shape[2], high.shape[3], high.shape[4])), 1 / opt.stop_scale)
    # print(opt.scale_factor)
    return high
...
#add
def create_reals_pyramid_high(high, opt):
    highs = []
    # use old rescaling method for harmonization
    # if opt.train_mode == "harmonization":
    #     for i in range(opt.stop_scale):
    ...
    for i in range(opt.stop_scale):
        ...
        curr_high = imresize(high,scale,opt)
