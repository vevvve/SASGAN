...
class TrilinearIntepolation(nn.Module):
    ...
    def sample_at_integer_locs(self, input_feats, index_tensor):
        ...
        sampled_in_2d = F.grid_sample(input=input_feats.contiguous().view(batch_size, num_chans*num_d, height, width),
...
if __name__=='__main__':
    ...
    parser.add_argument('--highres', default="Images/Generation/shayan_80.tif")
    ...
    #data = torch.rand(1, 3, 80, 80, 80)#(1, 32, 16, 128, 128)
    sampling_grid = (torch.rand(1, 40, 40, 3) - 0.5)*2.0#(1, 256, 256, 3)
    print('sampling_grid',sampling_grid.shape)
    #data = data.float().cuda(0)
    #high = high.float().cuda(0)
    sampling_grid = sampling_grid.float().cuda(0)
    trilinear_interpolation = TrilinearIntepolation().cuda(0)
    for i in range(1):
        t_start = time.time()
        interpolated_data = trilinear_interpolation(high, sampling_grid)
        # print('interpolated_data',interpolated_data.shape)
        # print(interpolated_data)
        img_save = functions.convert_image_np_2d(interpolated_data)
        tifffile.imsave('lowres.tiff', img_save[:, :, 0].astype(np.float32))
