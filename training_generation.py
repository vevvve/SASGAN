...
def train(opt):
    ...
    highs=functions.create_reals_pyramid_high(high,opt)

    real = functions.read_image(opt)
    real = functions.adjust_scales2image(real, opt)
    reals = functions.create_reals_pyramid(real, opt)#划分各个阶段
    ...
    for scale_num in range(opt.stop_scale+1):
        ...
        try:
            ...
            os.makedirs('{}/result'.format(opt.outf))
        except OSError:
                print(OSError)
                pass
        ...
        # print("1")
        fixed_noise, noise_amp, generator, d_curr = train_single_scale(d_curr, generator, reals, highs,fixed_noise, noise_amp, opt, scale_num, writer)
...
def train_single_scale(netD, netG, reals, highs,fixed_noise, noise_amp, opt, depth, writer):
    ...
    for iter in _iter:
        ...
        if depth<opt.train_stages- 1 :#6
            ...
            for j in range(opt.Dsteps):
                ...
                gradient_penalty = functions.calc_gradient_penalty(netD, high, fake, opt.lambda_grad, opt.device)
                errD_total = errD_real + errD_fake + gradient_penalty
                errD_total.backward()
                optimizerD.step()
        ...
        if iter>2000 and iter%100==0:
            img_noise1 = functions.convert_image_np(fake.detach())
            img_noise2 = functions.convert_image_np(rec.detach())
            tifffile.imsave('{}/result/fake_sample_fake_{}.tiff'.format(opt.outf, iter + 1),
                            img_noise1[:, :, :, 0].astype(np.float32))
            tifffile.imsave('{}/result/fake_sample_rec_{}.tiff'.format(opt.outf, iter + 1),
                            img_noise2[:, :, :, 0].astype(np.float32))
